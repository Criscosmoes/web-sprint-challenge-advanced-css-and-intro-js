...
function getArtistByIndex(array, index) {
  ...
  return `The artist and index ${index} is ${array[index].name}`; 
...
function get20s(data){
  ...
  for (let i = 0; i < data.length; i++){
    ...
    let date = data[i].years; 
    
    if(date[1] === "9" && date[8] === "9"){ // means that they were born in AND died in 19th
...
function removeArtist(arr, index) {
  ...
  return `Length of array is now ${arr.length}`; 
...
function addArtist(arr, object){
  ...
  console.log(addArtist(artists, {id: artists.length + 1, name: "Cristian Fernandez", years: "1997 - 2020", genre: "Web Design", nationality: "Mexican", bio: "One of the things I like to do in my free time is play soccer. I grew up playing soccer and will forever love it. "} ))
...
function randomize(/* Code here */){
 ...
 const filer = (arr) => {
  ...
  let newArr = arr.filter(cur => cur.years[1] == "9"); 
